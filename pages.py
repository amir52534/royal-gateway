# pages.py — Royal Panel Enterprise Theme
...
:root {
  ...
  --royal-gold-glow: rgba(255, 215, 0, 0.25);
}
...
.login-box {
  ...
  max-width: 420px;
  width: 100%;
  box-shadow: 0 25px 60px rgba(0,0,0,0.8), 0 0 80px rgba(255,215,0,0.05);
  position: relative;
  overflow: hidden;
}
.login-box::before {
...
<div class="login-box">
  <div class="royal-logo">
    <svg viewBox="0 0 100 100">
      ...
      <path d="M50,30 L58,42 L66,50 L58,58 L50,70 L42,58 L34,50 L42,42 Z" fill="url(#g)" opacity="0.3"/>
    </svg>
    <div class="royal-title">ROYAL</div>
    <div class="royal-sub">GATEWAY</div>
  </div>
  ...
  <div class="footer">ROYAL PANEL v1.0</div>
</div>
<script>
...
<head>
<meta charset="UTF-8"><meta name="viewport" content="width=device-width, initial-scale=1.0">
<title>Royal Panel · داشبورد</title>
<link rel="preconnect" href="https://fonts.googleapis.com">
<link rel="preconnect" href="https://fonts.gstatic.com" crossorigin>
<link href="https://fonts.googleapis.com/css2?family=Vazirmatn:wght@400;500;600;700;800&family=Inter:wght@400;500;600;700;800&display=swap" rel="stylesheet">
<link rel="stylesheet" href="https://cdn.jsdelivr.net/npm/@tabler/icons-webfont@3.19.0/dist/tabler-icons.min.css">
...
:root {
  ...
  --royal-light: #1A1A1A;
  --royal-gold-dark: #B8860B;
  --royal-gold-glow: rgba(255,215,0,0.15);
  --royal-gold-glow-strong: rgba(255,215,0,0.3);
  
  --bg: var(--royal-dark);
  --surface: rgba(26,26,26,0.92);
  --surface-glass: rgba(26,26,26,0.7);
  --text-primary: #FFFFFF;
  --text-secondary: rgba(255,215,0,0.6);
  ...
  --gradient-purple: linear-gradient(135deg, #FFD700, #FFA500);
  --radius-sm: 12px;
  --radius: 18px;
  --radius-lg: 24px;
  --radius-xl: 28px;
...
.sidebar-logo svg{width:36px;height:36px;filter:drop-shadow(0 0 20px rgba(255,215,0,0.2));flex-shrink:0}
.sidebar-logo-text{font-size:15px;font-weight:900;background:linear-gradient(135deg, #FFD700, #FFA500);-webkit-background-clip:text;-webkit-text-fill-color:transparent;background-clip:text}
...
.nav-item:hover{background:rgba(255,215,0,0.05);color:var(--royal-gold)}
.nav-item.active{background:linear-gradient(135deg, rgba(255,215,0,0.1), rgba(255,215,0,0.05));color:var(--royal-gold);border-right-color:var(--royal-gold);font-weight:700}
.nav-item.active i{color:var(--royal-gold)}
.nav-badge{margin-right:auto;background:rgba(255,215,0,0.12);color:var(--royal-gold);font-size:9px;padding:2px 7px;border-radius:20px;font-weight:700}
.sidebar-footer{padding:12px 16px;border-top:1px solid var(--border);flex-shrink:0}
.sidebar-user{display:flex;align-items:center;gap:10px;margin-bottom:10px}
.sidebar-avatar{width:38px;height:38px;border-radius:var(--radius-sm);background:var(--gradient);display:flex;align-items:center;justify-content:center;color:var(--royal-dark);font-size:16px;flex-shrink:0;box-shadow:0 4px 12px rgba(255,215,0,0.2)}
...
.theme-btn{display:flex;align-items:center;justify-content:center;gap:7px;width:100%;padding:9px;border:none;border-radius:var(--radius-sm);background:rgba(255,215,0,0.06);color:var(--text-secondary);font-family:inherit;font-size:11.5px;font-weight:600;cursor:pointer;transition:all .2s;border:1px solid var(--border)}
.theme-btn:hover{background:rgba(255,215,0,0.12);color:var(--royal-gold)}
.logout-btn{display:flex;align-items:center;justify-content:center;gap:7px;width:100%;padding:9px;border:none;border-radius:var(--radius-sm);background:rgba(255,0,0,0.06);color:#ff4444;font-family:inherit;font-size:11.5px;font-weight:600;cursor:pointer;transition:all .2s;margin-top:6px;border:1px solid rgba(255,0,0,0.1)}
.logout-btn:hover{background:rgba(255,0,0,0.12)}
...
.mobile-brand svg{width:28px;height:28px;filter:drop-shadow(0 0 10px rgba(255,215,0,0.15))}
.mobile-brand span{font-size:13px;font-weight:700;background:linear-gradient(135deg,#FFD700,#FFA500);-webkit-background-clip:text;-webkit-text-fill-color:transparent}
.mobile-actions{display:flex;gap:6px}
.mob-btn{background:rgba(255,215,0,0.06);border:1px solid var(--border);color:var(--text-secondary);width:34px;height:34px;border-radius:10px;font-size:16px;display:flex;align-items:center;justify-content:center;cursor:pointer}
...
.bell-btn{width:38px;height:38px;border-radius:var(--radius-sm);background:rgba(255,215,0,0.06);border:1px solid var(--border);color:var(--text-secondary);display:flex;align-items:center;justify-content:center;cursor:pointer;font-size:17px;transition:all .2s;position:relative}
.bell-btn:hover{background:rgba(255,215,0,0.12);color:var(--royal-gold)}
.bell-btn::after{content:'';position:absolute;top:8px;left:8px;width:7px;height:7px;border-radius:50%;background:#FFD700;border:2px solid var(--surface)}
...
.btn-primary{background:var(--gradient);color:var(--royal-dark);box-shadow:0 4px 20px rgba(255,215,0,0.2)}
.btn-primary:hover{transform:translateY(-2px);box-shadow:0 8px 30px rgba(255,215,0,0.35)}
.btn-primary:active{transform:translateY(0)}
.btn-ghost{background:rgba(255,215,0,0.06);color:var(--royal-gold);border:1px solid rgba(255,215,0,0.15)}
.btn-ghost:hover{background:rgba(255,215,0,0.12)}
.btn-danger{background:rgba(255,0,0,0.06);color:#ff4444;border:1px solid rgba(255,0,0,0.15)}
.btn-danger:hover{background:rgba(255,0,0,0.12)}
...
.btn-purple{background:rgba(255,215,0,0.08);color:var(--royal-gold);border:1px solid rgba(255,215,0,0.15)}
.btn-purple:hover{background:rgba(255,215,0,0.15)}
.btn-success{background:rgba(22,163,74,0.08);color:#4ADE80;border:1px solid rgba(22,163,74,0.15)}
...
.badge-blue{background:rgba(255,215,0,0.1);color:var(--royal-gold)}
.badge-green{background:rgba(22,163,74,0.1);color:#4ADE80}
.badge-red{background:rgba(255,0,0,0.1);color:#ff4444}
.badge-purple{background:rgba(255,215,0,0.08);color:#FFA500}
...
.stat-card:hover{transform:translateY(-3px);border-color:rgba(255,215,0,0.2);box-shadow:var(--shadow-hover)}
...
.stat-icon.blue{background:rgba(255,215,0,0.1);color:var(--royal-gold)}
.stat-icon.purple{background:rgba(255,215,0,0.08);color:#FFA500}
.stat-icon.green{background:rgba(22,163,74,0.1);color:#4ADE80}
.stat-icon.red{background:rgba(255,0,0,0.1);color:#ff4444}
...
.qa-btn:hover{background:var(--gradient);color:var(--royal-dark);border-color:transparent;box-shadow:0 6px 20px rgba(255,215,0,0.25);transform:translateY(-2px)}
...
.srv-bar-track{height:8px;border-radius:4px;background:rgba(255,215,0,0.08);overflow:hidden}
.srv-bar-fill{height:100%;border-radius:4px;transition:width .6s ease;position:relative}
.srv-bar-fill::after{content:'';position:absolute;inset:0;background:linear-gradient(90deg,transparent,rgba(255,255,255,0.2),transparent);width:50%;animation:shimmer 2s linear infinite}
.srv-bar-fill.blue{background:linear-gradient(90deg,#FFD700,#FFA500)}
.srv-bar-fill.purple{background:linear-gradient(90deg,#FFA500,#FF8C00)}
.srv-bar-fill.amber{background:linear-gradient(90deg,#FFD700,#FFD700)}
.srv-bar-fill.green{background:linear-gradient(90deg,#FFD700,#FFA500)}
...
.table-wrap th{background:rgba(255,215,0,0.05);font-size:10px;font-weight:800;color:var(--royal-gold);text-transform:uppercase;letter-spacing:.06em;padding:12px 14px;text-align:right;border-bottom:1px solid var(--border);position:sticky;top:0;z-index:1}
.table-wrap td{padding:12px 14px;font-size:12px;color:rgba(255,255,255,0.8);border-bottom:1px solid var(--border);transition:background .15s}
...
.pb-vless{background:rgba(255,215,0,0.15);color:var(--royal-gold)}
.pb-vmess{background:rgba(255,215,0,0.1);color:#FFA500}
.pb-trojan{background:rgba(255,215,0,0.12);color:#FFD700}
.pb-ss{background:rgba(22,163,74,0.1);color:#4ADE80}
.usage-bar-wrap{width:100%;min-width:100px}
.usage-bar{height:6px;border-radius:3px;background:rgba(255,215,0,0.08);overflow:hidden;margin-bottom:3px}
.usage-fill{height:100%;border-radius:3px;background:linear-gradient(90deg,#FFD700,#FFA500);transition:width .5s ease}
...
.status-dot.expired{color:#ff4444}
.exp-chip{font-size:9px;padding:2px 8px;border-radius:6px;font-weight:700}
.exp-ok{background:rgba(22,163,74,0.1);color:#4ADE80}
.exp-warn{background:rgba(245,158,11,0.1);color:#F59E0B}
.exp-bad{background:rgba(255,0,0,0.1);color:#ff4444}
...
.modal-head::before{content:'';position:absolute;top:0;right:0;width:150px;height:150px;background:radial-gradient(circle,rgba(255,215,0,0.06),transparent 70%);pointer-events:none}
...
.modal-close{width:32px;height:32px;border-radius:10px;background:rgba(255,215,0,0.06);border:1px solid var(--border);color:var(--text-secondary);display:flex;align-items:center;justify-content:center;cursor:pointer;font-size:16px;transition:all .2s}
.modal-close:hover{background:rgba(255,0,0,0.1);color:#ff4444;border-color:rgba(255,0,0,0.2)}
...
.form-input{width:100%;padding:10px 14px;border-radius:var(--radius);border:1.5px solid var(--border);background:rgba(255,255,255,0.05);backdrop-filter:blur(8px);color:var(--text-primary);font-family:inherit;font-size:12.5px;outline:none;transition:all .2s}
.form-input:focus{border-color:var(--royal-gold);box-shadow:0 0 0 3px rgba(255,215,0,0.1)}
.form-input::placeholder{color:var(--text-secondary);opacity:.5}
.form-row{display:flex;gap:10px;flex-wrap:wrap}
.form-row .form-field{flex:1;min-width:120px}
.form-select{width:100%;padding:10px 14px;border-radius:var(--radius);border:1.5px solid var(--border);background:rgba(255,255,255,0.05);backdrop-filter:blur(8px);color:var(--text-primary);font-family:inherit;font-size:12.5px;outline:none;cursor:pointer}
.form-select:focus{border-color:var(--royal-gold)}
.form-textarea{width:100%;padding:10px 14px;border-radius:var(--radius);border:1.5px solid var(--border);background:rgba(255,255,255,0.05);backdrop-filter:blur(8px);color:var(--text-primary);font-family:inherit;font-size:12.5px;outline:none;resize:vertical;min-height:70px}
.toggle-wrap{display:flex;align-items:center;gap:10px}
.toggle-switch{width:44px;height:24px;border-radius:12px;background:rgba(100,116,139,0.25);position:relative;cursor:pointer;transition:all .3s;border:none;flex-shrink:0}
...
.toggle-switch.on{background:linear-gradient(135deg,#FFD700,#FFA500)}
...
.cfg-card-item:hover{border-color:rgba(255,215,0,0.2);transform:translateY(-2px);box-shadow:var(--shadow-hover)}
...
.cfg-code{background:rgba(255,255,255,0.05);border:1px solid var(--border);border-radius:var(--radius-sm);padding:11px 13px;font-family:'Inter',monospace;font-size:10px;color:var(--royal-gold);word-break:break-all;line-height:1.7;max-height:80px;overflow-y:auto;margin-bottom:10px}
...
.log-icon-s.info{background:rgba(255,215,0,0.1);color:var(--royal-gold)}
.log-icon-s.ok{background:rgba(22,163,74,0.1);color:#4ADE80}
.log-icon-s.warn{background:rgba(245,158,11,0.1);color:#F59E0B}
.log-icon-s.err{background:rgba(255,0,0,0.1);color:#ff4444}
...
.toast.ok{border-color:rgba(255,215,0,0.3);background:rgba(255,215,0,0.08);color:var(--royal-gold)}
.toast.err{border-color:rgba(255,0,0,0.3);background:rgba(255,0,0,0.06);color:#ff4444}
...
<div class="bg-particles">
  ...
  <div class="bg-p" style="width:3px;height:3px;background:#FFA500;top:70%;left:40%;animation-duration:22s;animation-delay:-6s"></div>
  ...
  <div class="bg-p" style="width:3px;height:3px;background:#FFD700;top:80%;left:15%;animation-duration:20s;animation-delay:-3s"></div>
  <div class="bg-p" style="width:4px;height:4px;background:#FFA500;top:25%;left:55%;animation-duration:19s;animation-delay:-8s"></div>
</div>
...
<aside class="sidebar" id="sidebar">
  <div class="sidebar-logo">
    <svg width="36" height="36" viewBox="0 0 100 100"><defs><linearGradient id="sg1" x1="0%" y1="0%" x2="100%" y2="100%"><stop offset="0%" style="stop-color:#FFD700"/><stop offset="100%" style="stop-color:#FFA500"/></linearGradient></defs><rect x="10" y="10" width="80" height="80" rx="14" fill="none" stroke="url(#sg1)" stroke-width="2"/><path d="M50,25 L50,75 M25,50 L75,50" stroke="url(#sg1)" stroke-width="2.5" stroke-linecap="round"/><circle cx="50" cy="50" r="12" fill="none" stroke="url(#sg1)" stroke-width="1.5"/><circle cx="50" cy="50" r="4" fill="url(#sg1)"/><path d="M50,30 L58,42 L66,50 L58,58 L50,70 L42,58 L34,50 L42,42 Z" fill="url(#sg1)" opacity="0.2"/></svg>
    <div><div class="sidebar-logo-text">Royal Panel</div><div class="sidebar-logo-sub">Royal Gateway</div></div>
  </div>
  <div class="sidebar-nav">
    ...
    <div class="nav-item" data-page="logout" style="color:#ff4444"><i class="ti ti-logout"></i> خروج</div>
  </div>
        ...
        <div class="form-row" style="margin-bottom:12px">
          <div class="form-field">
            ...
            <div style="display:flex;flex-wrap:wrap;gap:8px;margin-top:6px">
              <label style="display:flex;align-items:center;gap:5px;font-size:12px;cursor:pointer;padding:6px 12px;background:rgba(255,215,0,0.06);border-radius:8px;border:1px solid var(--border)"><input type="checkbox" id="set-proto-vless" checked onchange="updateProtoCheckboxes()"> VLESS</label>
              <label style="display:flex;align-items:center;gap:5px;font-size:12px;cursor:pointer;padding:6px 12px;background:rgba(255,215,0,0.04);border-radius:8px;border:1px solid var(--border)"><input type="checkbox" id="set-proto-vmess" onchange="updateProtoCheckboxes()"> VMess</label>
              <label style="display:flex;align-items:center;gap:5px;font-size:12px;cursor:pointer;padding:6px 12px;background:rgba(255,215,0,0.05);border-radius:8px;border:1px solid var(--border)"><input type="checkbox" id="set-proto-trojan" onchange="updateProtoCheckboxes()"> Trojan</label>
              <label style="display:flex;align-items:center;gap:5px;font-size:12px;cursor:pointer;padding:6px 12px;background:rgba(255,215,0,0.04);border-radius:8px;border:1px solid var(--border)"><input type="checkbox" id="set-proto-reality" onchange="updateProtoCheckboxes()"> Reality</label>
            </div>
          </div>
        </div>
...
function renderUsersTable(links,tableId){
  ...
  tbody.innerHTML=links.map(function(l){
    ...
    var bc=pct>90?'#ff4444':pct>70?'#F59E0B':'#FFD700';
  ...
  try{
    ...
    if(d.hourly){
      ...
      if(trafficChart){trafficChart.data.labels=labels;trafficChart.data.datasets[0].data=vals;trafficChart.update()}
        ...
        grad.addColorStop(0,'rgba(255,215,0,0.45)');grad.addColorStop(1,'rgba(255,215,0,0)');
        trafficChart=new Chart(ctx,{
          type:'line',data:{labels:labels,datasets:[{data:vals,borderColor:'#FFD700',backgroundColor:grad,fill:true,tension:.4,pointRadius:0,borderWidth:2.5}]},
          options:{responsive:true,maintainAspectRatio:false,plugins:{legend:{display:false}},
            scales:{x:{grid:{display:false},ticks:{font:{size:9,color:'rgba(255,215,0,0.5)'}}},y:{grid:{color:'rgba(255,215,0,0.05)'},ticks:{font:{size:9,color:'rgba(255,215,0,0.5)'},callback:function(v){return v+' MB'}}}},
...
<title>Royal Panel · اشتراک</title>
<link rel="preconnect" href="https://fonts.googleapis.com">
<link rel="preconnect" href="https://fonts.gstatic.com" crossorigin>
<link href="https://fonts.googleapis.com/css2?family=Vazirmatn:wght@400;500;600;700;800&family=Inter:wght@400;500;600;700;800&display=swap" rel="stylesheet">
<link rel="stylesheet" href="https://cdn.jsdelivr.net/npm/@tabler/icons-webfont@3.19.0/dist/tabler-icons.min.css">
<style>
:root{{
  --royal-gold:#FFD700;--royal-dark:#0A0A0A;--royal-light:#1A1A1A;
  --text-primary:#FFFFFF;--text-secondary:rgba(255,215,0,0.6);
  --bg:var(--royal-dark);--surface:rgba(26,26,26,0.85);--surface-glass:rgba(26,26,26,0.7);
  --border:rgba(255,215,0,0.1);--radius:18px;--radius-lg:24px;--radius-xl:28px;
...
.bg-orb{{position:absolute;border-radius:50%;filter:blur(120px);opacity:.12;animation:float 14s ease-in-out infinite}}
.bg-orb:nth-child(1){{width:450px;height:450px;background:#FFD700;top:-150px;left:-100px}}
.bg-orb:nth-child(2){{width:350px;height:350px;background:#FFA500;bottom:-100px;right:-80px;animation-delay:-7s}}
...
.brand svg{{width:38px;height:38px;filter:drop-shadow(0 0 20px rgba(255,215,0,0.2))}}
.brand-name{{font-size:14px;font-weight:900;background:linear-gradient(135deg,#FFD700,#FFA500);-webkit-background-clip:text;-webkit-text-fill-color:transparent}}
...
.theme-toggle:hover{{background:rgba(255,215,0,0.1);color:var(--royal-gold)}}
...
.info-card::before{{content:'';position:absolute;top:0;right:0;width:180px;height:180px;background:radial-gradient(circle,rgba(255,215,0,0.06),transparent 70%);pointer-events:none}}
...
.info-stat{{text-align:center;padding:12px 8px;background:rgba(255,215,0,0.04);border-radius:var(--radius);border:1px solid var(--border)}}
...
.cfg-card:hover{{border-color:rgba(255,215,0,0.2);transform:translateY(-2px);box-shadow:var(--shadow-md)}}
...
.cfg-status.ok{{background:rgba(255,215,0,0.12);color:var(--royal-gold)}}
.cfg-status.no{{background:rgba(255,0,0,0.1);color:#ff4444}}
.cfg-code{{background:rgba(255,255,255,0.05);border:1px solid var(--border);border-radius:var(--radius);padding:12px 14px;font-family:'Inter',monospace;font-size:10px;color:var(--royal-gold);word-break:break-all;line-height:1.7;margin-bottom:10px;max-height:80px;overflow-y:auto}}
...
.btn i{{font-size:13px}}
.btn-p{{background:linear-gradient(135deg,#FFD700,#FFA500);color:var(--royal-dark);box-shadow:0 4px 14px rgba(255,215,0,0.25)}}
.btn-p:hover{{transform:translateY(-1px);box-shadow:0 6px 20px rgba(255,215,0,0.35)}}
.btn-ghost{{background:rgba(255,215,0,0.06);color:var(--royal-gold);border:1px solid rgba(255,215,0,0.15)}}
.btn-ghost:hover{{background:rgba(255,215,0,0.12)}}
...
.lock-icon{{width:64px;height:64px;border-radius:18px;background:rgba(255,215,0,0.1);border:1px solid rgba(255,215,0,0.2);display:flex;align-items:center;justify-content:center;margin:0 auto 18px;font-size:28px;color:var(--royal-gold)}}
...
.lock-input{{width:100%;padding:13px 44px;border-radius:var(--radius);border:1.5px solid var(--border);background:rgba(255,255,255,0.05);backdrop-filter:blur(8px);color:var(--text-primary);font-family:inherit;font-size:14px;text-align:center;outline:none;transition:all .2s;letter-spacing:.1em}}
.lock-input:focus{{border-color:var(--royal-gold);box-shadow:0 0 0 4px rgba(255,215,0,0.1)}}
...
.lock-err{{color:#ff4444;font-size:11px;margin-bottom:10px;min-height:18px}}
...
.toast.show{{opacity:1;transform:translateX(-50%) translateY(0)}}
.toast.ok{{border-color:rgba(255,215,0,0.3);color:var(--royal-gold)}}
...
<div class="wrap">
  ...
  <div class="footer">Royal Panel · Royal Gateway</div>
</div>
<script>
...
async function init(){{try{{var d=await loadData();if(d.locked){{renderLock(d.name);return}}renderContent(d)}}catch(e){{document.getElementById('root').innerHTML='<div class="empty"><i class="ti ti-alert-circle" style="color:#ff4444"></i><p>خطا در بارگذاری</p></div>'}}}}
init();
</script>
</body></html>"""